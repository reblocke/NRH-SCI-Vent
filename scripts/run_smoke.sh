...
temp_dir=$(mktemp -d "${TMPDIR:-/tmp}/nrh-smoke.XXXXXX")
status_file="$temp_dir/stata-status.txt"

cleanup() {
    rm -f -- "$status_file" \
        "$temp_dir/run_smoke.log" \
        "$temp_dir/run_smoke.smcl"
...
stata_command="do \"$repo_root/scripts/run_smoke.do\" \"$repo_root\" \"$status_file\""
for stata_arg in "$@"; do
    case "$stata_arg" in
        *\"*)
            echo "Smoke-launcher arguments may not contain a double quote." >&2
            exit 198
            ;;
    esac
...
if [ ! -f "$status_file" ]; then
    echo "Stata did not produce the smoke-run status sidecar." >&2
...
case "$stata_run_rc" in
    ''|*[!0-9]*)
        echo "Stata produced an invalid smoke-run status: $stata_run_rc" >&2
        exit 1
        ;;
esac

if [ "$stata_run_rc" -ne 0 ]; then
    echo "NRH smoke orchestration failed with Stata return code $stata_run_rc." >&2
    exit 1
fi
...
echo "NRH smoke orchestration completed successfully."
